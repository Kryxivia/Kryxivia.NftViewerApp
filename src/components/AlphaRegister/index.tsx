...
interface AlphaRegisterProps {
    chainId: number,
    accountId: string,
}

export const AlphaRegister: React.FC<AlphaRegisterProps> = ({ chainId, accountId}) => {
    ...
    useEffect(() => {
        const fetchRegistration = async () => {
            const result = await RegisterAlphaService.get(chainId, accountId);
        ...
        fetchRegistration();
    }, [chainId, accountId, setContactDiscord, setContactTelegram, setContactEmail]);

    async function register(e: any) {
        e.preventDefault();

        const isLoggedIn = WalletService.verifySessionIntegrity(accountId);
        if (isLoggedIn) {
            const result = await RegisterAlphaService.register(chainId, contactDiscord, contactTelegram, contactEmail, accountId);
            if (result) {
