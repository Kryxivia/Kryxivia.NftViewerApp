...
export const MintFirework = () => {
    ...
    const apiURL = CHAIN_INFO[CHAIN_ID].apiURL

    const { data: dataWinners, error } = useSWR(`${apiURL}/api/v1/alpha/winners`, fetcher);
    const { data: claimed } = useSWR(`${apiURL}/api/v1/alpha/claimed`, fetcher);

    const hasAccess = useHasAccess(ACCOUNT_ID, dataWinners);
    ...
    async function doMint(e: any) {
        resetFeedback();
        e.preventDefault();
        const result = await MintService.claimReward(CHAIN_ID, ACCOUNT_ID);
        if (result.txHash) {
        ...
        <>
            {CHAIN_ID !== 56 && (
                <div className="alert base">
                    Alpha Fireworks are only minted on BSC Mainnet. Please change your network.
                </div>
            )}
